...
const events = Object.create({}, {
    postEvent:{    
        ...
        ajax.post(titleInput.val(), seasonInput.val(), plotInput.val())
        .then(item => {
            reload();
        })
    }
    ...
    deleteEvent: {
        value: () => {
            ajax.delete(event.target.parentNode.id)
            .then(item =>{
                reload();
            })
    }
    ...
    editEvent: {
        value: () => {
            ...
            <br>
            <label>Plot:</label>
            <input id="editPlotInput" showId=${event.target.parentNode.id}></input>
    ...
    submitEdit: {
        value: () => {
            ...
            if(event.keyCode === 13){
            ajax.put(event.target.parentNode.id, title.val(), seasons.val(), plot.val())
            .then(item => {
                reload();
            })
        }
    ...
    checkBox: {
        value: () => {
            ...
            if ($(event.target).is(':checked')) {
                ajax.checked(event.target.parentNode.id)
                .then(item => {
                reload();
            })
            }
